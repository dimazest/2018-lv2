...
To study the collection in more detail, we manually searched the collection for \hl{9} topics of interest.
...
%
Following \cite{milajevs:2017:BUCC}, the \texttt{locations} parameter was set to the bounding box of Riga, the capital of Latvia. It is small enought to fit into Twitter API restrictions and covers about 40\% of the popilation of Latvia.%\footnotemark{}
%
...
\footnotetext{In this paper, by \hl{``tracking users''}, we mean that their user IDs where passed to the Twitter API to collect tweets that they created, retweets of those tweets, or the tweets that are replies by other users to their tweets. Refer to \href{https://developer.twitter.com/en/docs/tweets/filter-realtime/guides/basic-stream-parameters\#follow}{to the Twitter documentation} for a complete description. Keep in mind that this is different from the case when a user follows another user.}
